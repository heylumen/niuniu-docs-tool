...
| 5 | **合并多个工作簿** | 将多个 Excel 工作簿合并为一个（按表名或文件名归并所有工作表，支持 .xls / .xlsx） |
| 6 | **合并工作表** | 将单个工作簿内的多个工作表纵向合并为一个（可选追加来源列，支持 .xls / .xlsx） |
| 7 | **关键字提取** | 筛选包含指定关键字的行，支持多关键字和正则 |
| 8 | **工作簿拆分为 CSV** | Excel 工作簿（.xls / .xlsx）→ 多个 CSV（每表一个，UTF-8-BOM） |
| 9 | **超宽列拆分** | 超 256 列的工作表拆分为多个 ≤256 列的工作表（支持 .xls / .xlsx） |
...
> 当前最新版本 **v1.0.1**：新增 **Excel 97-2003（.xls）格式支持**，4 个 Excel 工具（合并工作簿 / 合并工作表 / 工作簿拆分 CSV / 超宽列拆分）现已同时支持 `.xls` 与 `.xlsx`。
